...
namespace WorldTree.SourceGenerator
{
	public static class SendRuleSupplementHelper
	{
		public static void GetMethod(StringBuilder Code, INamedTypeSymbol typeSymbol, INamedTypeSymbol? baseInterface)
		{
			...
			//生成调用方法
			Code.AppendLine(@$"		public static void {ClassName}<N{TypeArguments}>(this N self{genericTypeParameter}) where N : class, INode, AsRule<{ClassFullName}> {WhereTypeArguments} => NodeRuleHelper.{BaseName}(self, default({ClassFullName}){genericParameter});");
		}
	}
}