...
namespace WorldTree.SourceGenerator
{
	public static class CallRuleSupplementHelper
	{
		public static void GetMethod(StringBuilder Code, INamedTypeSymbol typeSymbol, INamedTypeSymbol? baseInterface)
		{
			...
			//生成调用方法
			Code.AppendLine(@$"		public static {outType} {ClassName}<N{TypeArguments}>(this N self{genericTypeParameter}) where N : class, INode, AsRule<{ClassFullName}> {WhereTypeArguments} => NodeRuleHelper.{BaseName}(self, default({ClassFullName}){genericParameter});");
		}
