...
namespace WorldTree
{
	...
	public static class ViewTypeManagerRule
	{
		class AddRule : AddRule<ViewTypeManager>
		{
			...
			{
				//收集所有实现了IWorldTreeNodeFieldInfoViewRule的法则的节点 
				if (self.Core.RuleManager.TryGetRuleGroup(typeof(IWorldTreeNodeFieldInfoViewRule).TypeToCore(), out RuleGroup ruleGroup))
				{
