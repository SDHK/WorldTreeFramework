...
namespace WorldTree.TreeDataFormatters
{
	public static class DecimalFormatterRule
	{
		...
		class Serialize : TreeDataSerializeRule<decimal>
		{
			...
			{
				if (typeMode != SerializedTypeMode.Value) self.WriteType(typeof(decimal));
		...
		class Deserialize : TreeDataDeserializeRule<decimal>
		{
			...
			{
				if (fieldNameCode != TreeDataCode.DeserializeSelfMode) { obj = self.ReadUnmanaged<decimal>(); return; }
