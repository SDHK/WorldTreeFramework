...
namespace WorldTree
{
	...
	///	YooAsset测试
	/// </summary>
	public partial class YooAssetTest : Node, ComponentOf<InitialDomain>
	, AsRule<Awake>
	{
		/// <summary>
		/// 资源包
		...
		[NodeRule(nameof(AddRule<YooAssetTest>))]
		private static void OnAddRule(YooAssetTest self)
		{
			//// 初始化资源系统
		...
		/// 编辑器模拟模式
		/// </summary>
		private async TreeTask InitializeYooAsset()
		{
			var initParameters = new EditorSimulateModeParameters();
			var simulateManifestFilePath = EditorSimulateModeHelper.SimulateBuild(EDefaultBuildPipeline.BuiltinBuildPipeline, "DefaultPackage");
			initParameters.SimulateManifestFilePath = simulateManifestFilePath;
			InitializationOperation a = await this.GetAwaiter(this.package.InitializeAsync(initParameters));

			AssetHandle handle = await this.GetAwaiter(this.package.LoadAssetAsync<GameObject>("MainWindow"));

			GameObject gameObject = handle.AssetObject as GameObject;
			this.Log($"YooAsset ??? : {gameObject.name}");
		}

		/// <summary>
		/// 单机运行模式
		/// </summary>
		private async TreeTask InitializeYooAsset1()
		{
		...
		/// 主机运行模式
		/// </summary>
		private async TreeTask InitializeYooAsset2()
		{
			...
			var initOperation = this.package.InitializeAsync(initParameters);
